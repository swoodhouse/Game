...
//extern "C" __declspec(dllexport) int minimax(int numVars, int ranges[], int minValues[], int numInputs[], int inputVars[], int numUpdates[], int inputValues[], int outputValues[], int numMutations, int numTreatments, int mutationVars[], int treatmentVars[], int apopVar) {
extern "C" __declspec(dllexport) int test(int numVars, int ranges[], int minValues[], int numInputs[], int inputVars[], int numUpdates[], int inputValues[], int outputValues[], int numMutations, int numTreatments, int mutationVars[], int treatmentVars[], int apopVar) {
	...
	Game game(std::move(minValuesV), std::move(rangesV), std::move(qn), std::move(mutationVarsV), std::move(treatmentVarsV), apopVar, 0, true);

	test1();
	maximum(game);
	oneZeroMaximum(game);
	findMax(game);
	renameMutVarsRemovingPrimes(game);
	backMax(game);
	backMin(game);
	untreat(game);
	unmutate(game);
...
/*
game - g++ -Icudd-3.0.0/cudd/ -Icudd-3.0.0/cplusplus/ -std=c++11 -W -Wall -Wextra -pedantic -o gameTest Test.cpp
*/
